...
BASE_ARGS = [
    "--model-type", "qnn",
    "--random-ops", "0",
    "--dataset", "blobs", # <--- CHANGED TO BLOBS
    "--vector-train", "50",
    "--vector-valid", "50",
    "--vector-test", "50",
    "--batch-size", "8",
...
def main():
    ...
    base_dir = Path(f"sweep_full_pipeline_blobs_{timestamp}")
    base_dir.mkdir(parents=True, exist_ok=True)

    print(f"--- STARTING FULL PIPELINE SWEEP (Blobs Dataset) ---")
    ...
    print(f"Ready to start the sweep on the Blobs dataset.")
    ...
    # 4. EXECUTION
    csv_path = base_dir / "master_results_full_pipeline_blobs.csv"
