...
# FIXED ARGUMENTS (Common to every run)
BASE_ARGS = [
    "--model-type", "qnn",
    "--random-ops", "0",
    "--dataset", "circles", # <--- CHANGED TO CIRCLES
    "--vector-train", "100",
    "--vector-valid", "100",
    "--vector-test", "100",
    "--batch-size", "8",
    "--epochs", "100",
    "--circles-noise", "0.3", # <--- CHANGED TO CIRCLES NOISE
    "--train_target",
    "--extra-feats",
    # "--plot-vector",
]
...
# 1. Feature Map Ranges (FM)
FM_KINDS = ["z", "zz", "eff_su2"]
N_WIRES_LIST = [2, 3, 4]
DEPTHS       = [2, 3, 4, 5, 6]
REPS_LIST    = [1, 2, 3, 4, 5]
PAD_MODES    = ["wrap", "repeatlast", "zero"]
FM_ENTANGLEMENTS = ["linear", "ring", "full"]
REDUCED_FM_EFF_TWOQ_OPS = ["cx", "cz"] 

# 2. Q-Layer Ranges (QL) - REDUCED FOR EFFICIENCY
QLAYER_ENT_KINDS = ["linear", "pairwise", "full"]
QLAYER_TWOQ_OPS  = ["cx", "crz", "rzz", "swap"]
QLAYER_REVERSE   = [False]
...
def main():
    ...
    # Updated directory name to reflect the dataset change
    base_dir = Path(f"sweep_full_pipeline_circles_{timestamp}")
    base_dir.mkdir(parents=True, exist_ok=True)

    print(f"--- STARTING FULL PIPELINE SWEEP (Circles Dataset) ---")
    ...
    print("Ready to start the sweep on the Circles dataset.")
    ...
    # 4. EXECUTION
    csv_path = base_dir / "master_results_full_pipeline_circles.csv"
