...
BASE_ARGS = [
    "--model-type", "qcnn",         # <--- CHANGED TO QCNN
    "--dataset", "mnist",
    "--random-ops", "0",
    
    # Data Settings (200 samples per split)
    "--vector-train", "100", # was 200
    "--vector-valid", "100",
    "--vector-test", "100",
...
def main():
    ...
    base_dir = Path(f"qcnn_sweep_100_100{timestamp}")
    base_dir.mkdir(parents=True, exist_ok=True)

    print(f"--- STARTING QCNN EXTENSIVE SWEEP (100 Samples) ---")
    ...
    # 3. EXECUTION
    csv_path = base_dir / "qcnn_extensive_results.csv"
