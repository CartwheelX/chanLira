...
# Required by CUDA >= 10.2 for deterministic cuBLAS matrix operations when
# torch.use_deterministic_algorithms(True) is enabled below. This must be set
# before importing torch or initializing CUDA.
...
# from feature_encoder_factory import build_feature_map
...
class HybridQNN(tq.QuantumModule):
    def __init__(self, cfg: QFCConfig):
        ...
        if cfg.fm_kind.lower() == "eff_su2":
            ...
            su2_name, su2_op = build_efficient_su2_oplist_qisk_new(
                D=self.D, n_wires=cfg.n_wires, reps=cfg.fm_eff_reps,
                single_ops=("ry","rz"),
                entanglement=cfg.fm_eff_ent_kind, twoq=cfg.fm_eff_twoq_op,
                pad_mode=cfg.fm_eff_pad_mod, alpha=cfg.fm_eff_alpha
            )
...
def reviewer_resource_summary(model: nn.Module, cfg: QFCConfig, model_type: str) -> dict:
    ...
    # its QNN reference, but it does not execute an encoder or variational
    # quantum circuit.  Do not report the reference circuit's analytic gates as
    # if they belonged to the classical baseline.
...
def main():
    ...
    if args.dataset == "mnist":
        ...
        mnist_signature = inspect.signature(MNIST)
        supports_arbitrary_kwargs = any(
            parameter.kind == inspect.Parameter.VAR_KEYWORD
            for parameter in mnist_signature.parameters.values()
        )
        if (
            "same_n_samples_each_class" in mnist_signature.parameters
            or supports_arbitrary_kwargs
        ):
    ...
    # here
    # exit()
    # Reset RNGs after dataset construction so model initialization/training
    ...
    if args.model_type != "mlp_qnn" and not os.environ.get("QURIFT_DISABLE_CIRCUIT_EXPORTS"):


        circuit_dir = Path("all_circ") 
        circuit_dir.mkdir(parents=True, exist_ok=True)
        safe_target_id = re.sub(r"[^A-Za-z0-9_.-]+", "_", args.target_id or f"run{args.run_id}")
