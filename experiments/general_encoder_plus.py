...
# #-------------------------------------------------------FEATURE ENCODERS new------------------------------------------------

# def _wrap_or_pad(k: int, D: int, pad_mode: str) -> Optional[int]:
#     """
#     Map index k into [0..D-1] according to pad_mode:
#       'wrap'       -> k % D
#       'repeatlast' -> D-1
#       'zero'       -> None (meaning: skip this gate)
#     """
#     if D <= 0:
#         return None
#     if k < D:
...
#     if pad_mode == "wrap":
#         return k % D
#     if pad_mode in ("repeatlast", "repeat_last"):
#         return D - 1
#     # 'zero' -> identity (skip)
#     return None
...
# # Z feature map: H on all wires then per-wire U1 phases
# def build_z_map_qiskit_ops(
#     n_wires: int,
#     D: int,
#     *,
...
#         # per-wire U1 phase
#         for w in range(n_wires):
#             k = _wrap_or_pad(idx, D, pad_mode)
#             idx += 1
#             if k is not None:
#                 ops.append({"input_idx": [k], "func": "u1", "wires": [w]})
#     name = name_prefix or f"{n_wires}x{reps}_hu1"
#     return name, ops

# # ZZ feature map: H, per-wire U1, then ZZ via CX–U1–CX
# def build_zz_map_qiskit_ops(
#     n_wires: int,
#     D: int,
#     *,
#     reps: Optional[int] = None,
#     entanglement: str = "ring",    # 'ring' (circular), 'linear', 'full'
#     pad_mode: str = "wrap",
#     name_prefix: Optional[str] = None,
# ) -> Tuple[str, List[Dict[str, Any]]]:
#     assert n_wires >= 1 and D >= 1
#     entanglement = entanglement.lower()
#     ent_tbl = {"ring": _pairs_ring, "circular": _pairs_ring, "linear": _pairs_linear, "full": _pairs_full}
#     if entanglement not in ent_tbl:
#         raise ValueError(f"invalid entanglement: {entanglement}")
#     pairs = ent_tbl[entanglement](n_wires)

#     per_rep = n_wires + len(pairs)
#     if reps is None:
#         reps = max(1, (D + per_rep - 1) // per_rep)
...
#         for w in range(n_wires):
#             ops.append({"input_idx": None, "func": "h", "wires": [w]})
#         # per-wire U1 phase (wrap/pad index)
#         for w in range(n_wires):
#             k = _wrap_or_pad(idx, D, pad_mode)
#             idx += 1
#             if k is not None:
#                 ops.append({"input_idx": [k], "func": "u1", "wires": [w]})
#         # ZZ layer via CX–U1–CX
#         for (a, b) in pairs:
#             k = _wrap_or_pad(idx, D, pad_mode)
#             idx += 1
#             if k is None:
...
#     entlabel = entanglement if entanglement != "circular" else "ring"
#     name = name_prefix or f"{n_wires}x{reps}_hppairs_{entlabel}"
#     return name, ops
...
# ------------------------------------------------------------------------------------------
# the following is for generating Pauli Mappings
# ------------------------------------------------------------------------------------------
import re
...
#------------------------------------USING Pauli to generate Z, ZZ, and gener Pauli-------------------------------------------------------
...
# def _ent_pairs(kind: str, n: int) -> List[Tuple[int,int]]:
#     k = kind.lower()
#     if k == "linear":   return _pairs_linear(n)
#     if k == "ring":     return _pairs_ring(n)
#     if k == "full":     return _pairs_full(n)
#     raise ValueError(f"Unknown entanglement '{kind}'")

# def _map_idx(idx: int, D: int, pad_mode: str) -> int:
#     """Return a safe feature index in [0, D-1] using padding."""
#     if D <= 0:
#         return 0
#     if idx < D:
#         return idx
...
#     if pm == "repeatlast":
#         return D - 1
#     # Fallback: keep it safe (use repeatlast if 'zero' was requested).
#     return D - 1
...
#     reps: Optional[int] = None,
#     pad_mode: str = "wrap",
#     phi: str = "prod"        # 'prod' or 'pi_minus' (kept for naming; indices stay the same)
# ) -> Dict[str, List[dict]]:
#     """
#     Qiskit-style ZZFeatureMap serialized as an op-list:
#       per repetition:
#         - H on every wire
#         - Z phase on every wire: u1(2*alpha*x_i)
#         - For each entangled pair (a,b): CX(a,b); u1(2*alpha*phi(x_a,x_b)) on b; CX(a,b)

#     Tiling: if D > n_wires, we do reps = ceil(D / n_wires) unless 'reps' is provided.
#     Padding: last repetition uses `pad_mode` ('wrap' or 'repeatlast') to map indices safely.

#     NOTE: Angles are not baked here (we only output indices). Your runtime should compute:
#       - single-qubit phase:   θ = 2*alpha * x[idx]
#       - pair phase:           θ = 2*alpha * ( x[idx_a]*x[idx_b] )  if phi='prod'
#                                θ = 2*alpha * ( (π - x[idx_a])*(π - x[idx_b]) )  if phi='pi_minus'
#     """
#     if n_wires < 1:
#         raise ValueError("n_wires must be >= 1")
#     per_rep_feats = max(1, n_wires)
#     total_reps = reps if reps is not None else max(1, math.ceil(D / per_rep_feats))

#     pairs = _ent_pairs(entanglement, n_wires)

#     oplist: List[dict] = []
#     for r in range(total_reps):
#         base = r * n_wires

#         # 1) Hadamards on all wires
#         for w in range(n_wires):
#             oplist.append({"input_idx": None, "func": "h", "wires": [w]})

#         # 2) Z single-qubit phases (P(2*x_i) == U1(2*x_i))
#         for w in range(n_wires):
#             idx = _map_idx(base + w, D, pad_mode)
#             oplist.append({"input_idx": [idx], "func": "u1", "wires": [w]})

#         # 3) ZZ pair phases (via CX; U1 on target; CX)
#         for a, b in pairs:
#             ia = _map_idx(base + a, D, pad_mode)
#             ib = _map_idx(base + b, D, pad_mode)
#             oplist.append({"input_idx": None,      "func": "cx", "wires": [a, b]})
#             oplist.append({"input_idx": [ia, ib],  "func": "u1", "wires": [b]})
#             oplist.append({"input_idx": None,      "func": "cx", "wires": [a, b]})

#     name = f"{n_wires}x{total_reps}_hrzz_{entanglement.lower()}_{phi.lower()}"
#     return name, oplist
...
# name, d = build_zz_oplist(n_wires=2, D=5, entanglement="linear", pad_mode="wrap")
