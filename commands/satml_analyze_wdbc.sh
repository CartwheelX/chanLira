...
RUNS="satml_runs/satml_wdbc_targeted"
RESULTS="satml_results/wdbc_targeted"
...
"${PYTHON_BIN}" -u reviewer_tools/threshold_mia_bootstrap.py \
  ...
  --fprs 0.01,0.05,0.10 2>&1 | tee satml_logs/wdbc_threshold_mia.log
...
"${PYTHON_BIN}" satml_tools/validate_added_datasets.py \
  --targets "${TARGETS}" --dataset breast_cancer_wdbc --wdbc-data data/wdbc/wdbc.csv.gz \
  --run-root satml_runs --attacks "${RESULTS}/threshold_mia/threshold_mia_raw.csv" \
