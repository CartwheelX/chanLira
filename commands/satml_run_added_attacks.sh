...
run_label_only() {
  ...
  "${PYTHON_BIN}" -u reviewer_tools/run_label_only_boundary_multigpu.py \
    ...
    --out-dir "${result_dir}/label_only" --n-member "${members}" \
    --n-nonmember "${nonmembers}" --anchors 16 --binary-steps 10 \
    --bootstrap 10000 --seed 2026 --gpus "${QURIFT_GPUS}" --jobs-per-gpu 1 \
    --cpu-threads 2 --resume 2>&1 | tee "satml_logs/${dataset}_label_only.log"
}
...
run_label_only fashion satml_targets/fashion_factorial_targets.csv 200 2000 satml_results/fashion_factorial
...
run_label_only wdbc satml_targets/wdbc_targeted_targets.csv 160 329 satml_results/wdbc_targeted
...
"${PYTHON_BIN}" -u satml_tools/analyze_paired_factorial.py \
  ...
  --attack-results satml_results/fashion_factorial/label_only/label_only_boundary_raw.csv \
...
"${PYTHON_BIN}" -u satml_tools/analyze_paired_factorial.py \
  ...
  --attack-results satml_results/wdbc_targeted/label_only/label_only_boundary_raw.csv \
